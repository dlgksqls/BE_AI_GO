...
def adress_list_print():
    ...
    for index in range(len(adress_list)):
        ...
        # 전화번호
        try:
            phone_number = driver.find_elements(By.CSS_SELECTOR, '.contact clickArea > .phone')
            phone_number = phone_number.__getitem__(index).find_elements(By.CSS_SELECTOR, 'p')
        except:
            phone_number = "Null"
...
# CSV 파일로 저장
output_csv_file = 'C:\Langchain\poat\langchain\관광지 주소.csv'
